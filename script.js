...
// الدخول اليدوي لتشغيل الصوت
enterSiteBtn.addEventListener('click', () => {
    // 1. تغيير شكل الزر
    enterSiteBtn.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin"></i> استعدي...';
    enterSiteBtn.style.pointerEvents = 'none';
    
    // 2. تشغيل الصوت وإعادة الفيديو
    if (splashVideo) {
        splashVideo.muted = false;
        splashVideo.currentTime = 0;
        splashVideo.play().catch(e => console.log("Audio play failed:", e));
    }
    
    // 3. الانتظار 2.2 ثانية لسماع الانترو ثم الدخول للموقع
    setTimeout(() => {
        hideSplash();
    }, 2200);
});

// تم إلغاء الاختفاء التلقائي حتى يضغط العميل على الزر ليسمع الصوت
